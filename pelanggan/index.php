...
if (isset($_POST['check_delivery_ajax']) && isset($_POST['phone'])) {
    $phone = $_POST['phone'];
    ...
    if ($result->num_rows > 0) {
        ...
        // First check if customer has processing orders
        $processingQuery = "SELECT id FROM pesanan WHERE id_pelanggan = ? AND status = 'diproses' ORDER BY waktu DESC LIMIT 1";
        ...
        if ($processingResult->num_rows > 0) {
            $processingOrder = $processingResult->fetch_assoc();
            $response['success'] = false;
            $response['processing'] = true;
            $response['order_id'] = $processingOrder['id'];
        } else {
            // If no processing orders, check for completed orders
            $completedQuery = "SELECT id FROM pesanan WHERE id_pelanggan = ? AND status = 'selesai' ORDER BY waktu DESC LIMIT 1";
            ...
            if ($completedResult->num_rows > 0) {
                $completedOrder = $completedResult->fetch_assoc();
                $response['success'] = true;
                $response['order_id'] = $completedOrder['id'];
            }
        }
    }
...
if (isset($_POST['track'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($interval->days > 7) {
                $message = "Tidak ada pesanan baru dalam 7 hari terakhir dari nomor telepon ini. Silahkan buat pesanan baru ke Zeea Laundry.";
            }
            ...
            $message = "Tidak ada pesanan ditemukan untuk nomor telepon ini.";
        }
    } else {
...
if (isset($_POST['view_history'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (count($orderHistory) == 0) {
            $message = "Tidak ada riwayat pesanan ditemukan untuk nomor telepon ini.";
        }
    } else {
        $message = "<div class='not-registered-alert'>
            ...
            <div class='not-registered-content'>
                <h5>Nomor Telepon Tidak Terdaftar</h5>
                <p>Nomor telepon yang Anda masukkan tidak terdaftar dalam sistem kami. Silakan periksa kembali nomor telepon Anda atau gunakan layanan jemput cucian untuk mendaftar.</p>
...
if (isset($_GET['redirect_to_track']) && isset($_GET['order_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $phone = $processingOrderDetails['telepon'];
    }
}
?>
...
<body>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="processing-order-alert">
                                        ...
                                        <div class="mt-3">
                                            <a href="index.php" class="btn btn-outline-primary w-100">
                                                <i class="fas fa-redo-alt me-2"></i> Cek Nomor Lain
                                            </a>
                                        </div>
                                    </div>
                                    ...
                                    <div class="order-card">
                                        <div class="order-header">
                                            <div class="order-id">Pesanan #<?php echo $processingOrderDetails['id']; ?></div>
                                    ...
                                    <form method="post" action="#track">
                                        <div class="mb-4">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control" id="phone" name="phone" value="<?php echo $phone; ?>" placeholder="Masukkan nomor telepon Anda" required>
                                            </div>
                                            <div class="form-text">Masukkan nomor telepon yang terdaftar di sistem kami</div>
                                        </div>
                                    ...
                                    <?php if ($orderDetails && empty($message)): ?>
                                        <div class="order-card">
                                            <div class="order-header">
                                                <div class="order-id">Pesanan #<?php echo $orderDetails['id']; ?></div>
                                            ...
                                            <div class="order-details">
                                                ...
                                                <div class="detail-item">
                                                    <div class="detail-label">Paket Laundry</div>
                                                    <div class="detail-value"><?php echo $orderDetails['nama_paket']; ?></div>
                                                </div>
                                                <div class="detail-item">
                                                    <div class="detail-label">Harga per Kilogram</div>
                                                    <div class="detail-value">Rp <?php echo number_format($orderDetails['harga_per_kg'], 0, ',', '.'); ?></div>
                                                </div>
                                                <div class="detail-item">
                                                    <div class="detail-label">Berat Cucian</div>
                                                    <div class="detail-value"><?php echo $orderDetails['berat']; ?> kg</div>
                                                </div>
                                                <div class="detail-item">
                                                    <div class="detail-label">Total Harga</div>
                                                    <div class="detail-value">Rp <?php echo number_format($orderDetails['harga'], 0, ',', '.'); ?></div>
                                                </div>
                                                <?php if ($orderDetails['harga_custom'] > 0): ?>
                                                <div class="detail-item">
                                                    <div class="detail-label">Harga Custom</div>
                                                    <div class="detail-value">Rp <?php echo number_format($orderDetails['harga_custom'], 0, ',', '.'); ?></div>
                                                </div>
                                                <?php endif; ?>
                                                <div class="detail-item">
                                                    <div class="detail-label">Status Pesanan</div>
                                            ...
                                            <?php if ($orderDetails['status'] == 'diproses' && $orderDetails['status_pembayaran'] == 'belum_dibayar'): ?>
                                                <div class="order-status">
                                                    <p>Pesanan Anda sedang diproses. Ingin bayar sekarang?</p>
                                            ...
                                            <?php elseif ($orderDetails['status'] == 'selesai' && $orderDetails['status_pembayaran'] == 'sudah_dibayar'): ?>
                                                <div class="order-status">
                                                    ...
                                                    <button type="button" class="btn btn-primary w-100" onclick="document.getElementById('delivery-tab').click()">
                                                        <i class="fas fa-truck-loading me-2"></i> Antar Cucian
                                                    </button>
                                                </div>
                                            <?php elseif ($orderDetails['status'] == 'selesai' && $orderDetails['status_pembayaran'] == 'belum_dibayar'): ?>
                                                <div class="order-status">
                                                    <p>Pesanan Anda telah selesai namun belum dibayar. Silakan lakukan pembayaran.</p>
                            ...
                            <div class="customer-card">
                                ...
                                <div class="card-body">
                                    <form method="post" action="#history">
                                        <div class="mb-4">
                                            <label for="history_phone" class="form-label">Nomor Telepon</label>
                                            <div class="input-group">
                                                <span class="input-group-text"><i class="fas fa-phone"></i></span>
                                                <input type="text" class="form-control" id="history_phone" name="history_phone" placeholder="Masukkan nomor telepon Anda" required>
                                            </div>
                                            <div class="form-text">Masukkan nomor telepon yang terdaftar di sistem kami</div>
                                        </div>
                                        <div class="text-center">
                                                ...
                                                <?php foreach ($orderHistory as $order): ?>
                                                    <div class="mobile-order-card">
                                                        ...
                                                        <div class="mobile-order-detail">
                                                            <div class="mobile-order-label">Paket:</div>
                                                            <div class="mobile-order-value"><?php echo $order['nama_paket']; ?></div>
                                                        </div>
                                                        <div class="mobile-order-detail">
                                                            <div class="mobile-order-label">Berat:</div>
                                                            <div class="mobile-order-value"><?php echo $order['berat']; ?> kg</div>
                                                        </div>
                                                        <div class="mobile-order-detail">
                                                            <div class="mobile-order-label">Harga:</div>
                                                            <div class="mobile-order-value">Rp <?php echo number_format($order['harga'], 0, ',', '.'); ?></div>
                                                        </div>
                            ...
                            <div class="customer-card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="alert alert-info mb-3">
                                        ...
                                        <p class="mt-2 mb-0">Layanan antar cucian hanya tersedia untuk pesanan yang telah selesai. Silakan masukkan nomor telepon Anda untuk memeriksa pesanan yang tersedia.</p>
                                    </div>
                                    ...
                                    <form method="post" action="#delivery" id="deliveryForm">
                                        <div class="mb-3">
                                            ...
                                            <div class="form-text">Gunakan nomor telepon yang terdaftar di sistem kami</div>
                                        </div>
                                        ...
                                        <div id="noCompletedOrders" class="alert alert-info mt-3" style="display: none;">
                                            ...
                                            <div class="mt-3">
                                                <button type="button" class="btn btn-outline-primary w-100" onclick="resetDeliveryForm()">
                                                    <i class="fas fa-redo-alt me-2"></i> Coba Nomor Lain
                                                </button>
                                            </div>
                                        </div>
                                    </form>
    ...
    <script>
        ...
        function resetDeliveryForm() {
            ...
            document.getElementById('delivery_phone').focus();
        }
        ...
        function checkDeliveryEligibility() {
            ...
            xhr.send('check_delivery_ajax=1&phone=' + encodeURIComponent(phone));
        }
